...
class Bot(VanirCog):
    ...
    async def ping(self, ctx: VanirContext):
        delays = {
            ...
            "\N{Earth Globe Americas} Web Requests": await timeit(self.bot.session.get, "https://example.com"),
            "\N{Elephant} PGSQL DB": await timeit(self.bot.db_currency.con.fetchval, "SELECT 0")
        }
        embed = ctx.embed(
    ...
    async def source(self, ctx: VanirContext, *, item: str = None):
        ...
        view = VanirView()
        view.add_item(discord.ui.Button(url=full_url, style=discord.ButtonStyle.success, label="View on GitHub"))
